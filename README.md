...
- **改期 / 重新分派 / 停用**：改期同步计划与期限；重新分派即时切换待办归属，原负责人立即失效；停用人员无法登录且不会丢历史，待办由物业重新分派或退回公共池。
...
端到端 + 并发领取自检（需要先在 8000 端口跑起后端并完成迁移/种子）：

```bash
cd backend
PORT=8000 python3 scripts/e2e_check.py
```

脚本覆盖：10 线程并发领取任务/整改单仅一人成功、完整异常→整改→驳回→复验→关闭闭环、改期/重新分派/停用归属同步、历史不丢失、超期升级与角色权限，共 40+ 项断言。
