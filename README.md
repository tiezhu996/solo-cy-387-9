...
`test_handoff_concurrency.py` 不依赖外部服务即可运行：它自建一次性 SQLite 库，迁移+种子后用 **gunicorn 多 worker（独立进程、独立数据库连接）** 拉起真实后端，用 `threading.Barrier` 让多名领取者与物业停用在同一刻发出真实 HTTP 请求，全程无 mock、无内存替身、无单连接串行化。同时覆盖两类竞争——「他人领取 vs 停用持有者」与**「本人领取 vs 停用本人」**（任务和整改单各一），后者验证领取与停用共享同一用户行级串行边界：领取在写事务首语句锁定本人用户行并复查启用，停用翻转并持锁到交接完成。按执行次序无关的不变量断言：最终归属唯一（成功领取后遇停用必被释放或转交，绝不停在已停用账号名下）、并发领取至多一人成功、失败方返回 409/403 且不改任务/整改单/历史、交接事件只作用于真实在办资源（时间线状态机重放）、期限与挂起升级单可回读、停用者不可登录。每轮自建用户与数据，结束自动终止进程并删除整库，可反复运行（可用 `ROUNDS/CLAIMERS/WORKERS` 调规模）。
