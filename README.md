...
`test_handoff_concurrency.py` 不依赖外部服务即可运行：它自建一次性 SQLite 库，迁移+种子后用 **gunicorn 多 worker（独立进程、独立数据库连接）** 拉起真实后端，用 `threading.Barrier` 让多名领取者与物业停用在同一刻发出真实 HTTP 请求，全程无 mock、无内存替身、无单连接串行化。同时覆盖三类竞争——「他人领取 vs 停用持有者」、**「本人领取 vs 停用本人」**、**「停用持有者并接管给 T vs 并发停用 T」**（任务和整改单各一）。多账号事务按用户 id 升序固定加锁：领取在写事务首语句锁定本人用户行并复查启用；停用翻转并持锁到交接完成；接管写入前在接管人锁内重新确认其仍启用，否则整笔回滚不留半完成记录。按执行次序无关的不变量断言：最终归属唯一（成功领取后遇停用必被释放或转交；接管成功后接管人被停用，待办必须再释放，绝不停在已停用账号名下）、并发领取至多一人成功、失败方返回 409/403 且不改任务/整改单/历史、交接事件只作用于真实在办资源（时间线状态机重放）、期限与挂起升级单可回读、停用者不可登录。每轮自建用户与数据，结束自动终止进程并删除整库和日志，可反复运行（可用 `ROUNDS/CLAIMERS/WORKERS` 调规模）。
